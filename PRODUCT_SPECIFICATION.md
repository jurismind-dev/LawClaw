...
- Setup 页在 provider 保存成功或 generic OAuth 成功后，会显式调用 `provider:setDefault`；Settings 页新增 / 登录 / 绑定成功后只保存配置，不自动切换当前 LawClaw provider
- 显式 `provider:setDefault` 仅更新 `lawclaw-main.model.primary`，不写全局默认模型；generic OAuth 成功仅落盘配置，不改写 OpenClaw 全局默认模型
- 当前 LawClaw provider 被删除，或在 API key 路径下失去最后一个有效凭据时，主进程会按“最近更新时间优先”自动补位到其他可用 provider；若无候选，则清空 LawClaw 当前 provider 并回退到 OpenClaw 默认模型
...
- 上述“保存后自动切换”为 Setup 专属行为；Settings 页保存成功后不会自动抢占当前 provider
...
- 提交前可执行 API Key 校验
- 新增 / 编辑 / OAuth / Jurismind 绑定成功后仅保存配置；只有显式点击“设为默认”时才调用 `provider:setDefault`
- 删除当前默认 provider，或删除 / 清空其最后一个有效 API Key 时，主进程会自动补位到最近更新且仍可用的其他 provider；若无候选则回退到 OpenClaw 默认模型
