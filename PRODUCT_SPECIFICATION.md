...
- LawClaw 仅对 `lawclaw-main` 维护“当前选中 provider 对应模型”；预设模板与启动迁移不再注入初始模型，若未选中任何 provider，则回退到 OpenClaw 默认模型机制
...
- 启动迁移仅修正 `lawclaw-main` 的专用约束（如 `id/name/workspace`），不再自动补齐 `lawclaw-main.model.primary`
- Setup 页在 provider 保存成功或 generic OAuth 成功后，会显式调用 `provider:setDefault`；仅当 `lawclaw-main.model.primary` 为空时才同步初始模型。Settings 页新增 / 登录 / 绑定成功后只保存配置，不自动切换当前 LawClaw provider
- 显式 `provider:setDefault` 会记录当前默认 provider，并按同步策略更新 `lawclaw-main.model.primary`；不写 OpenClaw 全局默认模型。generic OAuth 成功仅落盘配置，不改写 OpenClaw 全局默认模型
- 当前 LawClaw provider 被删除，或在 API key 路径下失去最后一个有效凭据时，主进程会按“最近更新时间优先”自动补位到其他可用 provider；仅当 `lawclaw-main.model.primary` 仍是系统受管值时才跟随补位。若无候选，则清空 LawClaw 当前 provider；仅在当前模型仍为受管值时同步清空 `lawclaw-main.model.primary`，否则保留用户自定义模型
...
- API Key 路径保存时执行：`provider:save`，随后由 Setup 页面显式调用 `provider:setDefault`
- generic OAuth 成功后会在 Setup 页面显式调用 `provider:setDefault`
- 上述“保存后设为默认”为 Setup 专属行为；仅当 `lawclaw-main.model.primary` 为空时才同步初始模型。Settings 页保存成功后不会自动抢占当前 provider
...
- 提交前可执行 API Key 校验
- 新增 / 编辑 / OAuth / Jurismind 绑定成功后仅保存配置；只有显式点击“设为默认”时才调用 `provider:setDefault`，并按显式切换策略同步 `lawclaw-main.model.primary`
- 删除当前默认 provider，或删除 / 清空其最后一个有效 API Key 时，主进程会自动补位到最近更新且仍可用的其他 provider；仅当 `lawclaw-main.model.primary` 仍是系统受管值时才同步补位。若无候选，则清空当前默认 provider；用户自定义模型保持不动
