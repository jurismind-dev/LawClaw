...
**劳有钳 (LawClaw)** 是一款面向律师和法律专业人士的 AI 助手桌面应用，由法义经纬 (Jurismind) 进行法律领域定制开发。

LawClaw 基于官方 [OpenClaw](https://github.com/OpenClaw) 运行时，提供易用、美观的桌面化 AI 编排体验。劳有钳在此强大基础上，针对法律行业进行深度定制，提供智能法律研究、文档分析、案件摘要和合同审查等专业功能。
...
法律专业人士值得拥有专为法律领域定制的AI工具。劳有钳的设计理念很简单：**强大的AI技术结合法律专业场景，提供高效、精准的法律服务支持。**
...
LawClaw 直接基于官方 **OpenClaw** 核心构建。无需单独安装，我们将运行时嵌入应用内部，提供开箱即用的无缝体验。
...
### 🌙 自适应主题
支持浅色模式、深色模式或跟随系统主题。劳有钳自动适应您的偏好设置。
...
首次启动劳有钳时，**设置向导** 将引导您完成以下步骤：

1. **语言与区域** – 配置您的首选语言（中文/English）
2. **AI 供应商** – 配置受支持的模型供应商（含 `Jurismind（法义经纬）`、`Moonshot - Code Plan（月之暗面-编程包月）`、`GLM - Code Plan（智谱-编程包月）`）。Code Plan 供应商仅需填写 API Key。
3. **消息平台（可选）** – 在设置向导中连接渠道。`Jurismind（法义经纬）` 目前以 **即将支持** 形式展示，`Feishu / Lark` 优先展示。
4. **基础组件安装** – 安装 uv、托管 Python 运行时与默认工具链依赖。
5. **验证** – 在进入主界面前确认配置。
...
劳有钳采用 **双进程架构**，将 UI 层与 AI 运行时操作分离：
...
我们欢迎社区的各种贡献！无论是修复 Bug、开发新功能、改进文档还是翻译——每一份贡献都让 LawClaw 变得更好。
