...
interface QrCodeConstructor {
  new (typeNumber: number, errorCorrectionLevel: unknown): QrMatrix;
}

interface BaileysEventBus {
  ...
  removeAllListeners(event: 'creds.update' | 'connection.update'): void;
}
...
let cachedRuntime: BaileysRuntime | null = null;

function loadWhatsAppRuntime(): BaileysRuntime {
  if (cachedRuntime) {
  ...
  if (
    ...
    throw new Error('WhatsApp runtime is incomplete in the bundled OpenClaw dependencies.');
  }

  const QRCode = require(join(qrcodeTerminalPath, 'vendor', 'QRCode', 'index.js')) as QrCodeConstructor;
  ...
  private async connectToWhatsApp(accountId: string): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        pino = baileysRequire('pino') as (...args: unknown[]) => Record<string, unknown>;
      } catch (e) {
        console.warn('[WhatsAppLogin] Could not load pino from baileys, trying root', e);
        try {
          pino = require('pino') as (...args: unknown[]) => Record<string, unknown>;
        } catch {
