...
if (bridgedExtensionRuntimePackages.length > 0) {
  console.log(
    `[dev] Bridged OpenClaw staged extension runtime deps: ${bridgedExtensionRuntimePackages.join(', ')}`
  );
}
if (patchedCompatPackages.length > 0) {
  console.log(`[dev] Patched OpenClaw require-compatible packages: ${patchedCompatPackages.join(', ')}`);
}
