...
describe('openclaw acpx jurismind preset patch', () => {
  ...
  it('configures top-level ACP policy to target Jurismind xHigh through acpx', () => {
    ...
    expect(patch.acp?.defaultAgent).toBe(CODEX_ACP_HARNESS_ID);
    expect(patch.acp?.allowedAgents).toEqual([CODEX_ACP_HARNESS_ID]);
  });
  ...
  it('relies on OpenClaw/acpx built-in Codex adapter without a LawClaw command override', () => {
    const patch = readPatch();
...
describe('openclaw acpx jurismind preset (end-to-end migration)', () => {
  ...
  it('merges Jurismind xHigh agent and acpx config into a fresh openclaw.json', async () => {
    ...
    expect(jurismind?.runtime?.acp?.agent).toBe(CODEX_ACP_HARNESS_ID);
    ...
    expect(merged.acp?.defaultAgent).toBe(CODEX_ACP_HARNESS_ID);
    expect(merged.acp?.allowedAgents).toEqual([CODEX_ACP_HARNESS_ID]);
    ...
    expect(acpx?.config?.agents).toBeUndefined();
  });
  ...
  it('renames the managed legacy Codex preset and removes schema-invalid ACP runtime', async () => {
    ...
    expect(merged.acp?.backend).toBe('acpx');
    expect(merged.acp?.defaultAgent).toBe(CODEX_ACP_HARNESS_ID);
    expect(merged.acp?.allowedAgents).toContain(CODEX_ACP_HARNESS_ID);
  });

  it('migrates beta Jurismind xHigh ACP aliases back to OpenClaw native Codex', async () => {
    const { openclawDir, lawclawDir } = createFixture();
    ...
    const userConfig = {
      ...
      acp: {
        enabled: true,
        backend: 'acpx',
        defaultAgent: 'jurismind-xhigh',
        allowedAgents: ['jurismind-xhigh', 'codex'],
      },
      plugins: {
        entries: {
          acpx: {
            ...
            config: {
              agents: {
                ...
                codex: {
                  command: 'env -u OPENAI_API_KEY -u CODEX_API_KEY npx @zed-industries/codex-acp@^0.11.1',
                },
              },
            },
          },
    ...
    expect(merged.acp?.defaultAgent).toBe(CODEX_ACP_HARNESS_ID);
    expect(merged.acp?.allowedAgents).toEqual([CODEX_ACP_HARNESS_ID]);
    expect(merged.plugins?.entries?.acpx?.config?.agents).toBeUndefined();
  });
